...
class CharRNN:
    ...
    def __init__(self, num_classes, num_seqs=64, num_steps=50,
        ...
        print(" ---------------------- LSTM_ATTENTION model is builded ---------------------- ")
    ...
    def build_lstm(self):
        ...
        with tf.name_scope('lstm'):
            ...
            with tf.name_scope("Attention_layer"):
                self.attention_output, self.alphas = self.attention_layer(self.lstm_outputs, self.lstm_size)

            # 注意力输出用于连接到softmax层
            x = tf.reshape(self.attention_output, [-1, self.lstm_size])  # 重新整形为适合softmax层的形状
