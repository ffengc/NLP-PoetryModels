...
class CharRNN_RNN:
    ...
    def build_inputs(self):
        with tf.name_scope('inputs'):
            ...
            if self.use_embedding is False:
                self.rnn_inputs = tf.one_hot(self.inputs, self.num_classes)  # 使用one_hot编码
            else:
                with tf.device("/cpu:0"):  # 在CPU上执行嵌入层计算
                    embedding = tf.get_variable('embedding', [self.num_classes, self.embedding_size])
                    self.rnn_inputs = tf.nn.embedding_lookup(embedding, self.inputs)  # 查找输入的嵌入表示
    ...
    def build_rnn(self):
        ...
        with tf.name_scope('rnn'):
            # 创建前向和后向的LSTM层
            cell_fw = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(self.rnn_size, self.keep_prob) for _ in range(self.num_layers)])
            cell_bw = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(self.rnn_size, self.keep_prob) for _ in range(self.num_layers)])
            ...
            with tf.name_scope("Attention_layer"):
                self.attention_output, self.alphas = self.attention_layer(self.rnn_outputs, self.rnn_size)

            # 连接和重塑输出，准备连接softmax层
            seq_output = tf.concat(self.attention_output, 1)  # 连接输出
            x = tf.reshape(seq_output, [-1, 2 * self.rnn_size])  # 重塑形状为[批次大小 * 序列长度, 2 * lstm大小]

            with tf.variable_scope('softmax'):
                softmax_w = tf.Variable(tf.truncated_normal([2 * self.rnn_size, self.num_classes], stddev=0.1))  # 权重
