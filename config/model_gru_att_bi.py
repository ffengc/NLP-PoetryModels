...
class CharRNN_GRU:
    ...
    def build_inputs(self):
        with tf.name_scope('inputs'):
            ...
            if self.use_embedding is False:
                self.gru_inputs = tf.one_hot(self.inputs, self.num_classes)  # 使用one_hot编码
            else:
                with tf.device("/cpu:0"):  # 在CPU上执行嵌入层计算
                    embedding = tf.get_variable('embedding', [self.num_classes, self.embedding_size])
                    self.gru_inputs = tf.nn.embedding_lookup(embedding, self.inputs)  # 查找输入的嵌入表示
    ...
    def build_gru(self):
        ...
        with tf.name_scope('gru'):
            # 创建前向和后向的GRU层
            cell_fw = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(self.gru_size, self.keep_prob) for _ in range(self.num_layers)])
            cell_bw = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(self.gru_size, self.keep_prob) for _ in range(self.num_layers)])
            ...
            # 使用bidirectional_dynamic_rnn来创建双向GRU
            ...
            # 将前向和后向的输出合并
            self.gru_outputs = tf.concat([output_fw, output_bw], axis=2)
            ...
            seq_output = tf.concat(self.attention_output, 1)  # 连接输出
            x = tf.reshape(seq_output, [-1, 2 * self.gru_size])  # 重塑形状为[批次大小 * 序列长度, 2 * GRU大小]

            with tf.variable_scope('softmax'):
                softmax_w = tf.Variable(tf.truncated_normal([2 * self.gru_size, self.num_classes], stddev=0.1))  # 权重
